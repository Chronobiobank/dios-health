-- TipTraQ PDF upload pipeline + proxy DLMO (v1)
-- Run in Supabase SQL Editor or via: supabase db push

-- ─── PDF uploads storage bucket ─────────────────────────────────────────────

insert into storage.buckets (id, name, public)
values ('tiptraq-reports', 'tiptraq-reports', false)
on conflict (id) do nothing;

-- Storage policy: users can only access their own reports
...
create policy "Clinicians can read linked patient tiptraq nights"
  on public.tiptraq_nights for select
  using (
    exists (
      select 1 from public.clinician_patients cp
      where cp.clinician_id = auth.uid()
        and cp.patient_id = tiptraq_nights.patient_id
...
create policy "Clinicians can read linked patient dlmo profiles"
  on public.dlmo_profiles for select
  using (
    exists (
      select 1 from public.clinician_patients cp
      where cp.clinician_id = auth.uid()
        and cp.patient_id = dlmo_profiles.patient_id
        and cp.status = 'active'
    )
  );
